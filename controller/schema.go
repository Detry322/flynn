...
func migrateDB(db *sql.DB) error {
  ...
  ELSE
    PERFORM pg_notify('events', NEW.event_id || '');
  END IF;
	RETURN NULL;
    END;
$$ LANGUAGE plpgsql`,
    ...
    BEGIN
        ...
        INSERT INTO events (app_id, object_id, object_type, data) VALUES(NEW.app_id, NEW.app_id || ':' || NEW.release_id, 'scale', hstore_to_json(NEW.processes));
        RETURN NULL;
    END;
$$ LANGUAGE plpgsql`,
